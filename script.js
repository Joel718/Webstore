...
function showProduct(mainIndex, subIndex) {
    ...
    for (i = 0; i < productList.length; i++) {
        if (productList[i].mainCategory == mainIndex + 1 && productList[i].subCategory == subIndex)
            $str += "<div class='productitem' onclick='largeview(" + (mainIndex + 1) + "," + subIndex + ")'><img src='img/doge.jpg' class='photo'><span class='name'>" + productList[i].prodName + "</span><span class='price'>price: " + productList[i].prodPrice + "</span></div>";
    }
    $('.content').append($str);
...
// Produktinfo
function largeview(m, s) {
    for (i = 0; i < productList.length; i++) {
        if (productList[i].mainCategory == m && productList[i].subCategory == s) {
            ...
            $('.largeview').children('.photo').text(productList[i].image);
        }
    }
